...
def get_avg_wait(hospital_df, hospital_name):
    hospital_index = hospital_df[hospital_df['hospital_name'] == hospital_name].index[0]
    avg_wait_time = hospital_df.loc[hospital_index, 'avg_wait_time']
...
def estimate_business(hospital_df, hospital_name, current_time, year_start_time=0,
    ...
    avg_wait_time = get_avg_wait(hospital_df, hospital_name)
    ...
    max_capacity = avg_wait_time * 1.5
...
'''
things to check: 
    - time counter - i think its pretty manky right now, sorry. 
    - how/where is the hospital_data.csv saved 
